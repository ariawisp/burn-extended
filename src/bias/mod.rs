...
pub fn sinks_from_per_head<B: Backend>(
    per_head: Tensor<B, 1>,
    kv_heads: usize,
    groups: usize,
) -> Tensor<B, 2> {
...
pub fn alibi_bias<B: Backend>(
    ...
    let s = if let Some(s) = slopes {
        s.to_vec()
    } else {
        default_slopes(n_heads)
    };
    let slopes =
        Tensor::<B, 1>::from_floats(TensorData::new(s, [n_heads]), device).reshape([n_heads, 1, 1]);
    let q = Tensor::<B, 1, Int>::arange(0..q_len as i64, device)
        .float()
        .reshape([q_len, 1]);
    let k = Tensor::<B, 1, Int>::arange(0..k_len as i64, device)
        .float()
        .reshape([1, k_len]);

...
pub fn generate_causal_mask_1d<B: Backend>(
    seq_len: usize,
    device: &B::Device,
) -> Tensor<B, 2, Bool> {
...
pub fn generate_windowed_causal_mask<B: Backend>(
    ...
    if let Some(w) = window_len {
        ...
        mask.reshape([1, seq_length, seq_length])
            .repeat_dim(0, batch_size)
    } else {
        base.reshape([1, seq_length, seq_length])
            .repeat_dim(0, batch_size)
    }
}
...
pub fn generate_chunked_causal_mask_1d<B: Backend>(
    ...
    for i in 0..seq_len {
        ...
        let start = if num_left_chunks < 0 {
            ...
            ((chunk_idx as isize - num_left_chunks).max(0) as usize) * chunk_size
        };
